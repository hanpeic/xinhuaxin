...
(function($, window, undefined) {
    ...
    $(function() {
      ...
      if ($(window).width() > 767) {
        $("#inputForm:not(.nofocus) input[type=text]:not([readonly]):not([disabled]):not(.nofocus):eq(0)").focus()
      }
      ...
      $(document).on("click", ".addTabPage", function(e) {
        ...
        if (href && href != "" && href != "blank") {
          title = $.trim(title || js.text("tabpanel.newTabPage"));
          ...
          if ($this.parent().hasClass("treeview")) {
            ...
            top.window.location.hash = href.replace("#", "") + "#" + title
          }
          return false
        }
        return true
        ...
        if (!js.layer) {
          if (confirm(message)) {
            if (typeof urlOrFun == "function") {
              ...
              js.ajaxSubmit(urlOrFun, data, callback, dataType, async, loadingMessage)
            }
          }
          return
        }
...
+function(d) {
  ...
  f.prototype.expand = function() {
    var n = d.Event(j.expanded);
    var m = this.options.collapseIcon;
    var k = this.options.expandIcon;
    d(this.element).removeClass(i.collapsed);
    d(this.element).find(a.tools).find("." + k).removeClass(k).addClass(m);
    var l = this;
    d(l.element).find(a.body + ", " + a.footer).slideDown(l.options.animationSpeed, function() {
      d(l.element).trigger(n)
    })
  }
  ;
  f.prototype.collapse = function() {
    ...
    var k = this.options.expandIcon;
    d(this.element).find(a.tools).find("." + n).removeClass(n).addClass(k);
    var m = this;
    d(m.element).find(a.body + ", " + a.footer).slideUp(m.options.animationSpeed, function() {
      d(m.element).addClass(i.collapsed);
      d(m.element).trigger(l)
    }
      .bind(m))
  }
  ;
  f.prototype.remove = function() {
    var l = d.Event(j.removed);
    var k = this;
    d(k.element).slideUp(this.options.animationSpeed, function() {
      d(k.element).trigger(l);
      d(k.element).remove()
    })
  }
  ;
